...
class Particle {
  constructor(
    ...
    this.x = x < treshold ? treshold : (x > (canvas.width - treshold) ? canvas.width - treshold : x);
    this.y = y < treshold ? treshold : (y > (canvas.height - treshold) ? canvas.height - treshold : y);
    this.red = red;
    this.green = green;
    this.blue = blue;
    this.alpha = 0.05;
